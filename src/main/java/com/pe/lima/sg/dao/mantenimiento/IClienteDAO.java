...
public interface IClienteDAO extends BaseOperacionDAO<TblCliente, Integer> {
	...
	@Query(value = "select *  from mae.tbl_cliente where numero_documento = :numeroDocumento AND estado = '1'  and codigo_entidad = :codigoEmpresa", nativeQuery = true)
	List<TblCliente> listarClientexDocumentoxEmpresa(@Param("numeroDocumento") String strNumeroDocumento, @Param("codigoEmpresa") Integer intCodigoEmpresa);
